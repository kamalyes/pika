...
@router.post("/gconfig/insert", name="增加全局配置")
...
@router.delete("/gconfig/delete", name="删除全局配置")
async def delete_gconfig(id: int, user_info=Depends(Permission(RoleEnum.ADMIN)), session=Depends(async_db_session)):
    await GConfigDao.delete_record_by_id(session, user_info['emp_no'], id, log=True)
    return PikaResponse.success()


@router.post("/gconfig/update", name="更新全局配置")
