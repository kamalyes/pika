...
from app.models.test_result import PikaTestResult


class TestResultDao(object):
    log = PikaLogger("TestResultDao")
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    result = PikaTestResult(report_id, case_id, case_name, status,
    ...
    @staticmethod
    async def list(report_id: int) -> List[PikaTestResult]:
        try:
            async with async_session() as session:
