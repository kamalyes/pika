...
@dao(PikaConstructor, PikaLogger("ConstructorDao"))
class ConstructorDao(PikaMapper):

    @staticmethod
    async def list_constructor(case_id: int) -> List[PikaConstructor]:
        """
        ...
        try:
            async with async_session() as session:
                sql = select(PikaConstructor).where(PikaConstructor.case_id == case_id, PikaConstructor.deleted_at == 0) \
                    .order_by(PikaConstructor.index, PikaConstructor.update_date)
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(PikaConstructor).where(PikaConstructor.case_id == data.case_id, PikaConstructor.name == data.name,
                                                    PikaConstructor.deleted_at == 0)
                    result = await session.execute(sql)
                    if result.scalars().first() is not None:
                        raise Exception(f"{data.name}已存在")
                    constructor = PikaConstructor(**data.dict(), operator=operator)
    ...
    async def update_constructor(data: ConstructorForm, operator: int) -> None:
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(PikaConstructor).where(PikaConstructor.id == data.id)
                    result = await session.execute(sql)
    ...
    async def delete_constructor(cls, id: int, operator: int) -> None:
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(PikaConstructor).where(PikaConstructor.id == id)
                    result = await session.execute(sql)
                    query = result.scalars().first()
                    if query is None:
                ...
                async with session.begin():
                    for item in data:
                        await session.execute(
                            update(PikaConstructor).where(PikaConstructor.id == item.id).values(index=item.index))
        except Exception as e:
    ...
    async def get_constructor_tree(cls, name: str, suffix: bool) -> List[dict]:
        try:
            async with async_session() as session:
                # 获取所有构造参数
                search = [PikaConstructor.public == True, PikaConstructor.suffix == suffix, PikaConstructor.deleted_at == 0]
                if name:
                    search.append(PikaConstructor.name.like("%{}%".format(name)))
                query = await session.execute(select(PikaConstructor).where(*search))
                ...
                query = await session.execute(select(PikaTestCase).where(PikaTestCase.id.in_(temp.keys())))
    ...
    @staticmethod
    async def get_constructor_data(id_: int) -> PikaConstructor:
        """
        根据构造方法id获取构造方法数据
        :param id_:
        :return:
        """
        async with async_session() as session:
            query = await session.execute(select(PikaConstructor).where(PikaConstructor.id == id_, PikaConstructor.deleted_at == 0))
    ...
    async def get_case_and_constructor(constructor_type: int, suffix: bool) -> List[dict]:
        ...
        async with async_session() as session:
            ...
            query = await session.execute(
                select(PikaConstructor).where(
                    PikaConstructor.suffix == suffix,
                    PikaConstructor.type == constructor_type,
                    PikaConstructor.public == True,
                    PikaConstructor.deleted_at == 0))
            ...
            # 二次查询，查出有前置条件的case
            query = await session.execute(
                select(PikaTestCase).where(PikaTestCase.id.in_(constructors.keys()), PikaTestCase.deleted_at == 0))
