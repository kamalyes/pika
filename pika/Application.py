...
def init_scheduler():
    ...
    engine_options = {"pool_recycle": PikaAppConfig.MYSQL_POOL_RECYCLE, "encoding": PikaAppConfig.MYSQL_CHARSET}
    job_store = {
        "default": SQLAlchemyJobStore(
            url=PikaAppConfig.SQLALCHEMY_DATABASE_URI,
...
async def websocket_endpoint(websocket: WebSocket, emp_no: str):
    ...
    try:
        ...
        msg_records = await PikaNotificationDao.list_messages(
            msg_type=MessageTypeEnum.all.value, receiver=emp_no, msg_status=MessageStateEnum.unread.value,
        )
...
if __name__ == "__main__":
    uvicorn.run(
        app="Application:pika", host=PikaAppConfig.PIKA_BACKEND_HOST, port=PikaAppConfig.PIKA_BACKEND_PORT, reload=True,
    )
