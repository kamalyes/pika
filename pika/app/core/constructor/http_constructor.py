    ...
    async def run(cls, executor, env, index, path, params, constructor: ConstructorModel, **kwargs):
        try:
            ...
            if isinstance(headers, str):
                headers = cls.safe_json_loads(headers)
            client = await AsyncRequest.client(
                url=url, content_type=data.get("content_type"), headers=headers, request_body=data.get("request_body"),
            )
