...
class Executor(object):
    ...
    async def run(
        self, env: str, case_id: str, params_pool: dict = None, request_param: dict = None, path: str = "主case",
    ):
        """
        开始执行测试用例
        Args:
    ...
    async def run_single(
        env: str, result_data, report_id, case_id, params_pool: dict = None, path="主case", retry_minutes=0,
    ):
        """
        ...
        if not test_data:
            await Executor.run_with_test_data(
                env, result_data, report_id, case_id, params_pool, {}, path, "默认数据", retry_minutes=retry_minutes,
            )
        else:
                ...
                for m in msg_types:
                    if int(m) == NoticeTypeEnum.EMAIL:
                        ...
                        return EmailManger.send_email(
                            content=content, subject=subject, addressee=[r.get("email") for r in users],
                        )
                    if int(m) == NoticeTypeEnum.DINGDING:
                        ...
                        await ding.send_msg(
                            f"{PikaGlobalVarEnum.LOWER_HUMP_APP_NAME}测试报告", render_markdown, None, users,
                        )
        return None
