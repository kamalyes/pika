...
def get_env():
    my_env = Environment()
    for func in dir(PikaFunction):
        # 过滤掉内置方法
        if func.startswith("__"):
            continue
        my_env.globals[func] = getattr(PikaFunction, func)
...
if __name__ == "__main__":
    # import jinja2
    # template = jinja2.Template("""hello {{nan.abc[0].orderId}} """)
    # from jinja2 import Environment
    # from jinja2.runtime import Context
    # env = Environment()
    a = "2023-09-15 12:00:00.223"
    temp = """{{uuid()}}"""
    c = Render.render(dict(), temp)
    print(c)
    js_dat = {"nan": {"abc": [{"ni_zai_gan_ma": "12356", "orderId": 55}]}}
