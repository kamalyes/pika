...
from app.crud import PikaMdWrapper, PikaWrapper, db_connect
...
class ApiTestCaseDao(PikaWrapper):
    ...
    async def get_case_children(cls, case_id: str):
        ...
        return [{"key": f"asserts_{d.id}", "title": d.name, "case_id": case_id} for d in data]
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    query = await session.execute(
                        select(ApiTestCaseModel).where(
                            ApiTestCaseModel.id == test_case.id, ApiTestCaseModel.delete_flag == 0,
                        ),
                    )
    ...
    async def query_test_case_by_constructors(cls, constructors: List[ConstructorModel]):
        ...
        try:
            # 找到所有用例名称为
            constructors = [
                PikaJsonEncoder.safe_json_loads(x.constructor_json).get("case_id") for x in constructors if x.type == 0
            ]
            async with async_session() as session:
                sql = select(ApiTestCaseModel).where(
                    ApiTestCaseModel.id.in_(constructors), ApiTestCaseModel.delete_flag == 0,
                )
                result = await session.execute(sql)
    ...
    async def query_test_case_out_parameters(
        session, case_list: List[ApiTestCaseVariablesSchema], case_set=None, var_list=None,
    ):
        """
        根据前置场景id获取对应的参数
        :param case_list:
        ...
        if len(case_list) == 0:
            return None
        if case_set is None:
            case_set = {c.case_id for c in case_list}
        if var_list is None:
            var_list = {}
        cs_list = [c.case_id for c in case_list]
        step_case = []
        name_dict = {c.case_id: c.step_name for c in case_list}
        # 获取用例的前后置步骤和出参
        out = select(ApiTestCaseOutParametersModel).where(
            ApiTestCaseOutParametersModel.case_id.in_(cs_list), ApiTestCaseOutParametersModel.delete_flag == 0,
        )
        parameters = await session.execute(out)
        for p in parameters.scalars().all():
            var_list.append({"stepName": name_dict[p.case_id], "name": "${%s}" % p.name})
        sql = select(ConstructorModel).where(ConstructorModel.case_id.in_(cs_list), ConstructorModel.delete_flag == 0)
        steps = await session.execute(sql)
        for s in steps.scalars().all():
            if s.value:
                var_list.append({"stepName": s.name, "name": "${%s}" % s.value})
                continue
    ...
    async def collect_data(cls, case_id: str, data: List):
        ...
        """
        # 先获取数据构造器(前置条件)
        pre = {"id": f"pre_{case_id}", "label": "前置条件", "children": []}
        suffix = {"id": f"suffix_{case_id}", "label": "后置条件", "children": []}
        await cls.collect_constructor(case_id, pre, suffix)
        data.append(pre)

        # 获取断言
        asserts = {"id": f"asserts_{case_id}", "label": "断言", "children": []}
    ...
    async def collect_constructor(cls, case_id, parent_id, suffix):
        ...
        for c in constructors:
            temp = {"id": f"constructor_{c.id}", "label": f"{c.name}", "children": []}
    ...
    async def collect_asserts(cls, case_id, parent_id):
        ...
        for a in asserts:
            temp = {"id": f"assert_{a.id}", "label": f"{a.name}", "children": []}
            parent_id.get("children").append(temp)
    ...
    async def query_user_case_rank(cls, session: AsyncSession = None) -> List:
        ...
        for i, q in enumerate(query.all()):
            user, count = q
            ans.append({"id": user, "count": count, "rank": i + 1})
        return ans
    ...
    async def query_weekly_user_case(cls, operator: str, start_date: datetime, finished_date: datetime) -> List:
        ...
        async with async_session() as session:
            async with session.begin():
                ...
                for _i, q in enumerate(query.all()):
                    date, count = q
    ...
    async def fill_data(cls, start_date: datetime, finished_date: datetime, data: dict):
        ...
        while start <= finished_date:
            ...
            ans.append({"date": date, "count": data.get(date, 0)})
