...
class ApiTestCaseDao(PikaWrapper):
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    query = await session.execute(
                        select(ApiTestCaseModel).where(
                            ApiTestCaseModel.id == test_case.id,
                            ApiTestCaseModel.delete_flag == 0,
                        ),
                    )
        ...
        try:
            async with async_session() as session:
                ...
                parameters = await ApiTestCaseOutParametersDao.select_list(
                    case_id=case_id,
                    _sort=(asc(ApiTestCaseOutParametersModel.id),),
                )
                return {
                    "asserts": asserts,
        ...
        try:
            ...
            async with async_session() as session:
                sql = select(ApiTestCaseModel).where(
                    ApiTestCaseModel.id.in_(constructors),
                    ApiTestCaseModel.delete_flag == 0,
                )
                result = await session.execute(sql)
    ...
    async def query_test_case_out_parameters(
        ...
        out = select(ApiTestCaseOutParametersModel).where(
            ApiTestCaseOutParametersModel.case_id.in_(cs_list),
