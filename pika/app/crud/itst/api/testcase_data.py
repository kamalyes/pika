...
class ApiTestCaseDataDao(PikaWrapper):
    ...
    async def update_testcase_data(cls, form: ApiTestCaseDataSchema, operator: str):
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(ApiTestCaseDataModel).where(
                        ApiTestCaseDataModel.id == form.id, ApiTestCaseDataModel.delete_flag == 0,
                    )
                    result = await session.execute(sql)
                    query = result.scalars().first()
                    if query is None:
    ...
    async def delete_testcase_data(cls, id: str, operator: str):
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(ApiTestCaseDataModel).where(
                        ApiTestCaseDataModel.id == id, ApiTestCaseDataModel.delete_flag == 0,
                    )
                    result = await session.execute(sql)
                    query = result.scalars().first()
                    if query is None:
    ...
    async def list_testcase_data(cls, case_id: str):
        ...
        try:
            async with async_session() as session:
                sql = select(ApiTestCaseDataModel).where(
                    ApiTestCaseDataModel.case_id == case_id, ApiTestCaseDataModel.delete_flag == 0,
                )
                result = await session.execute(sql)
    ...
    async def list_testcase_data_by_env(cls, env: str, case_id: str) -> List[ApiTestCaseDataModel]:
        try:
            async with async_session() as session:
                sql = select(ApiTestCaseDataModel).where(
                    ApiTestCaseDataModel.case_id == case_id,
