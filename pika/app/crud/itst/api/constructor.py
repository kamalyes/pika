                    ...
                    result.append(
                        {
                            ...
                            "children": [
                                {"key": f"constructor_{x.id}", "title": x.name, "value": f"constructor_{x.id}"}
                                for x in v
                            ],
                        },
                    )
        ...
        async with async_session() as session:
            ...
            query = await session.execute(
                select(ApiTestCaseModel).where(
                    ApiTestCaseModel.id.in_(constructors.keys()),
            ...
            for q in query.scalars().all():
                ...
                ans.append(
                    {"title": q.name, "key": f"case_id_{q.id}", "disabled": True, "children": constructors[q.id]},
                )
        return ans
