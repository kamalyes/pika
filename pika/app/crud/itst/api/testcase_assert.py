        ...
        try:
            async with async_session() as session:
                query = await session.execute(
                    select(ApiTestCaseAssertsModel).where(
                        ApiTestCaseAssertsModel.case_id == case_id, ApiTestCaseAssertsModel.delete_flag == 0,
                    ),
    ...
    async def update_test_case_asserts(cls, form: TestCaseAssertsSchema, operator: str) -> ApiTestCaseAssertsModel:
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(ApiTestCaseAssertsModel).where(
                        ApiTestCaseAssertsModel.id == form.id, ApiTestCaseAssertsModel.delete_flag == 0,
                    )
                    result = await session.execute(sql)
                    data = result.scalars().first()
                    if data is None:
    ...
    async def delete_test_case_asserts(cls, id: str, operator: str) -> None:
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = select(ApiTestCaseAssertsModel).where(
                        ApiTestCaseAssertsModel.id == id, ApiTestCaseAssertsModel.delete_flag == 0,
                    )
                    result = await session.execute(sql)
                    data = result.scalars().first()
                    if data is None:
