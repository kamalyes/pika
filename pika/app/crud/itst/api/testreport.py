...
class ApiTestReportDao(PikaWrapper):
    ...
    async def query(cls, report_id: str):
        ...
        try:
            async with async_session() as session:
                sql = (
                    select(ApiTestReportModel, ApiTestPlanModel.name)
                    .outerjoin(ApiTestPlanModel, ApiTestPlanModel.id == ApiTestReportModel.plan_id)
                    .where(ApiTestReportModel.id == report_id)
                )
                data = await session.execute(sql)
                if data is None:
        ...
        try:
            async with async_session() as session:
                sql = (
                    ...
                    .where(ApiTestReportModel.start_date.between(start_date, finished_date))
                    .order_by(desc(ApiTestReportModel.start_date))
                )
