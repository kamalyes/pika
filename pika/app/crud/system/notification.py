...
class PikaNotificationDao(PikaWrapper):
    ...
    async def list_messages(cls, msg_type: int, msg_status: int, receiver: str):
        ...
        if msg_type == MessageTypeEnum.others:
            ...
            async with async_session() as session:
                # 找到3个月内的消息
                default_condition = [NotificationModel.delete_flag == 0, NotificationModel.create_date >= ninety_days]
                if msg_type == MessageTypeEnum.broadcast:
                    ...
                    conditions = [
                        *default_condition,
                        or_(
                            NotificationModel.msg_type == MessageTypeEnum.broadcast.value,
                            and_(
                ...
                sql = (
                    select(NotificationModel, BroadcastReadUserModel)
                    .outerjoin(
                        BroadcastReadUserModel,
                        and_(
                            NotificationModel.id == BroadcastReadUserModel.notification_id,
                            BroadcastReadUserModel.read_user == receiver,
                        ),
                    )
                    .where(*conditions)
                    .order_by(NotificationModel.create_date.desc())
                )
    ...
    async def delete_message(cls, session, msg_id: List[str], receiver: str):
        async with session.begin():
            ...
            ).values(delete_date=0, update_date=datetime.now(), update_emp_no=receiver)
