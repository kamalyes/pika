...
class ProjectDao(PikaWrapper):
    ...
    async def list_project(
        cls, operator: str, operator_identity: int, paging, name: str = None,
    ) -> (List[ProjectModel]):
        """
        查询/获取项目列表
        Args:
        ...
        try:
            ...
            async with async_session() as session:
                if operator_identity != RoleEnum.ADMIN:
                    ...
                    search.append(
                        or_(
                            ProjectModel.id.in_(project_list), ProjectModel.owner == operator, ProjectModel.private == 0,
                        ),
                    )
                if name:
    ...
    async def list_project_id_by_user(cls, session, operator, role):
        ...
        ans = set()
        # 找到包含用户的角色
        sel_role2user = select(ProjectRoleModel.project_id).where(ProjectRoleModel.member_no == operator)
        roles = await session.execute(sel_role2user)
        for r in roles.all():
            ans.add(r[0])
        # 找到未删除的项目
        sel_not_del_dt = select(ProjectModel.id).where(
            or_(ProjectModel.private is False, ProjectModel.owner == operator), ProjectModel.delete_flag == 0,
        )
    ...
    async def is_project_admin(cls, session, project_id: str, operator: str):
        query = await session.execute(select(ProjectModel.owner).where(ProjectModel.id == project_id))
        ...
        async with async_session() as session:
            async with session.begin():
                data = await session.execute(
                    select(ProjectModel).where(ProjectModel.name == name, ProjectModel.delete_flag == 0),
                )
    ...
    @classmethod
    async def update_avatar(cls, project_id: str, operator: str, operator_identity: int, file_url: [str, int]):
        try:
            async with async_session() as session:
                async with session.begin():
                    query = await session.execute(
                        select(ProjectModel).where(ProjectModel.id == project_id, ProjectModel.delete_flag == 0),
                    )
    ...
    async def update_project(
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    query = await session.execute(
                        select(ProjectModel).where(ProjectModel.id == id, ProjectModel.delete_flag == 0),
                    )
                    data = query.scalars().first()
                    if data is None:
                        raise Exception("项目不存在")
    ...
    async def query_user_project(emp_no: str) -> int:
        ...
        async with async_session() as session:
            async with session.begin():
                ...
                query = await session.execute(
                    select(ProjectRoleModel).where(
                        ProjectRoleModel.delete_flag == 0, ProjectRoleModel.member_no == emp_no,
                    ),
                )
...
class ProjectRoleDao(PikaWrapper):
    ...
    async def list_project_by_user(cls, emp_no: str) -> List[str]:
        ...
        try:
            async with async_session() as session:
                data = await session.execute(
                    select(ProjectRoleModel.project_id).where(
                        ProjectRoleModel.member_no == emp_no, ProjectRoleModel.delete_flag == 0,
                    ),
                )
    ...
    async def list_role(cls, project_id: str) -> List[ProjectRoleModel]:
        try:
            async with async_session() as session:
                query = await session.execute(
                    select(ProjectRoleModel).where(
                        ProjectRoleModel.project_id == project_id, ProjectRoleModel.delete_flag == 0,
                    ),
                )
    ...
    async def judge_permission(
        cls, session: AsyncSession, project_id: str, emp_no: str, project_role: int, project_admin: bool,
    ) -> None:
        """
        判断用户是否有某个项目的权限
        Args:
        ...
        if project.owner != emp_no:
            if project_admin and project_role == RoleEnum.MANAGER:
                raise AccessException(detail="不能修改组长的权限")
            query = await session.execute(
                select(ProjectRoleModel).where(
            ...
            if updater_role is None or updater_role.project_role == RoleEnum.MANAGER:
                raise AccessException(detail="对不起,你没有权限")

    @staticmethod
    async def access(
        operator: str, operator_identity: str, roles: List[ProjectRoleModel], project: ProjectModel = None,
    ):
        ...
        if not any(r.operator == operator for r in roles):
            raise AccessException(detail="没有权限访问项目")
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    ...
                    await cls.has_permission(
                        original.project_id, original.project_role, operator, operator_identity, True, session=session,
                    )
    ...
    async def delete_project_role(cls, prole_id: str, operator: str, operator_identity: int) -> None:
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    role = await cls.query_record(session=session, id=prole_id, delete_flag=False)
                    if role is None:
