...
from app.exceptions import DbUpdateError, DbDeleteError
...
class PikaWrapper(object):
    ...
    async def update_by_map(cls, operator, *condition, **kwargs):
        try:
            async with async_session() as session:
                async with session.begin():
                    sql = update(cls.__model__).where(*condition) \
                        .values(**kwargs,
    ...
    @classmethod
    @RedisHelper.up_cache("dao")
    async def delete_records(cls, session, operator, id_list: List[str], column="id", description=None,
                             log=True):
        try:
            for id_ in id_list:
