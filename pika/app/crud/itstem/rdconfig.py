...
from app.middleware.xredis import PikaRedisManager, RedisHelper
...
class PikaRedisConfigDao(PikaWrapper):
    ...
    async def execute_command(cls, command: str, **kwargs):
        ...
        if not redis_config.cluster:
            client = PikaRedisManager.get_single_node_client(
                redis_config.id, redis_config.addr, redis_config.password, redis_config.db,
            )
        else:
