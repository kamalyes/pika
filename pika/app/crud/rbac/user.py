...
from custard.core import DataHand, Kerberos, MockHelper
...
from app.crud.rbac import client_ip, regex_register_str
...
from app.crud.system.email import Email
...
class UserDao(PikaWrapper):
            ...
            async with session.begin():
                users = await session.execute(
                    select(UserModel).where(
                        or_(UserModel.username == register_model.username, UserModel.email == register_model.email),
                    ),
                )
                ...
                user = UserModel(
                    emp_no=emp_no, username=register_model.username, identity=identity, email=register_model.email,
                )
                session.add(user)
                await session.flush()
                await session.refresh(user)
                pwd_valid_date = PikaGlobalVarEnum.PWD_VALID_DATE
                user_admin = SysUserAdminModel(
                    uid=user.id,
                    emp_no=user.emp_no,
    ...
    async def account_status_verify(**kwargs):
        ...
        try:
            compare_time = Moment.compare_time(
                kwargs["pwd_valid_date"], Moment.get_now_time(PikaGlobalVarEnum.TIME_FORMATTING_YTDHMS),
            )
        except Exception as e:
            raise SystemException(code=ExcCodeEnum.FIELD_TYPE_ERROR, detail=f"密码有效期对比失败!具体错误原因:{e}")
        if compare_time is False:
            raise SystemException(code=ExcCodeEnum.PASSWORD_HAS_EXPIRED, detail="密码已过期,请修改后进行登录!")
            ...
            async with session.begin():
                ...
                if err_pwd_count >= ValidTimeEnum.ERR_PWD_COUNT.value:
                    ...
                    sql = (
                        ...
                        .where(SysUserAdminModel.uid == kwargs["uid"])
                        .values({"err_pwd_count": int(err_pwd_count + 1)})
                    )
    ...
    async def generate_uuid_jwt(cls, **kwargs):
        ...
        try:
            ...
            jwt_encode_result = Kerberos.jwt_encode(
                secret_key=PikaAppConfig.JWT_SECRET_KEY, target_value=target_value, seconds=kwargs["valid_time"],
            )
            return f"{jwt_encode_result}.{uuid4_}".replace("4", str(random.randint(5, 9)))
        except Exception as uuid_jwt_err:
            raise SystemException(code=ExcCodeEnum.JWT_ENCODE_ERROR, detail=f"加密失败,具体原因{uuid_jwt_err}")
    ...
    async def update_last_login_field(cls, **kwargs):
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                sql = (
                    update(SysUserAdminModel)
                    .where(or_(SysUserAdminModel.id == uid, SysUserAdminModel.emp_no == emp_no))
                    .values(
                        {
    ...
    async def update_avatar(cls, emp_no, avatar):
        async with async_db_session_generator() as session:
            async with session.begin():
                sql = update(UserModel).where(UserModel.emp_no == emp_no).values({"avatar": avatar})
                await session.execute(sql)
    ...
    async def query_user_info(cls, **kwargs):
        uid, emp_no = kwargs.get("uid", None), kwargs.get("emp_no", None)
        async with async_db_session_generator() as session:
            users = await session.execute(select(UserModel).where(or_(UserModel.id == uid, UserModel.emp_no == emp_no)))
            user_admins = await session.execute(
                select(SysUserAdminModel).where(or_(SysUserAdminModel.uid == uid, SysUserAdminModel.emp_no == emp_no)),
            )
            user, user_admin = users.scalars().first(), user_admins.scalars().first()
            if user and user_admin:
                user_infos = DataHand.chain_all(
                    [PikaResponse.model_to_dict(user), PikaResponse.model_to_dict(user_admin)],
                )
                # 屏蔽字段
    ...
    async def account_login(cls, request, oauth2_login):
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                sql = select(UserModel).where(
                    or_(
                        UserModel.username == oauth2_login.username,
                ...
                if user:
                    ...
                    if old_token and PikaAppConfig.JWT_MPOP:
                        ...
                        uuid_jwt = await cls.generate_uuid_jwt(
                            emp_no=user.emp_no, valid_time=valid_time, password=user_admin.password,
                        )
                        await cls.uuid_jwt_sync_redis(
                            key=f"{RedisKeyEnum.AUTH_TOKEN}:{user.emp_no}", value=uuid_jwt, ex=valid_time,
                        )
                else:
        ...
        return PikaResponse.success(
            data=DataHand.chain_all([user_infos, {"token": uuid_jwt}]), message=PromptEnum.LOGIN_SUCCEED.value,
        )
    ...
    async def email_login(cls, request, oauth2_login):
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                ...
                user = users.scalars().first()
                if user:
                    await cls.has_mail_verify_code(verify_code=oauth2_login.dynamic_code, model=2, emp_no=user.emp_no)
                    user_admins = await session.execute(
        ...
        async with async_db_session_generator as session:
            async with session.begin():
                sql = (
                    update(SysUserAdminModel)
                    .where(or_(SysUserAdminModel.emp_no == user.emp_no))
                    .values({"err_pwd_count": 0})
                )
                await session.execute(sql)
                session.execute()
        user_infos = await cls.query_user_info(uid=user.id)
    ...
    async def account_logout(cls, request, oauth2_logout):
        ...
        await cls.update_last_logout_field(emp_no=oauth2_logout.emp_no, last_logout_ip=last_logout_ip)
    ...
    async def add_user(cls, request, operator):
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                ...
                user = UserModel(
                    emp_no=emp_no, username=request.username, identity=request.identity, email=request.email,
                )
                session.add(user)
            await session.refresh(user)
            user_admin = SysUserAdminModel(
    ...
    async def update_user_info(cls, modify_user_info, operator):
        ...
        await regex_register_str(
            email=modify_user_info.email, mobile=modify_user_info.mobile, gender=modify_user_info.gender,
        )
        update_info = {
            "avatar": modify_user_info.avatar,
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                sel_sql = select(UserModel).where(
                    and_(
                        or_(
                            UserModel.email == modify_user_info.email,
    ...
    async def query_user_info_list(db, request):
        if str(request.query_type) == "0":
            ...
            return await paginate(
                ...
                select(UserModel).where(
                    or_(
                        UserModel.id == request.id,
                        UserModel.emp_no == request.emp_no,
                    ...
                    and_(UserModel.create_date >= request.create_date, UserModel.update_date <= request.update_date),
                ),
            )
        else:
    ...
    async def has_dynamic_code(dynamic_code):
        ...
        if dynamic_code in PikaGlobalVarEnum.VERIFY_CODE_WHITE_LIST or has_key:
            ...
            raise SystemException(code=ExcCodeEnum.DYNAMIC_ERROR, detail="验证码已过期或不存在")
    ...
    async def get_verifycode(request, user_info):
        if request.models is VerifyCodeEnum.FORGET_PWD.value:
            await Email.rand_mail_code(
                emp_no=user_info["emp_no"], username=user_info["username"], addressee=user_info["email"],
            )
        else:
    ...
    async def send_email_verify_code(request):
        if request.model == 2:
            ...
            await Email.rand_mail_code(
                emp_no=exists_users.emp_no, username=exists_users.username, addressee=exists_users.email, model=2,
            )
    ...
    async def update_pwd(**kwargs):
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                sql = update(SysUserAdminModel).where(SysUserAdminModel.emp_no == emp_no).values(update_info)
                await session.execute(sql)
    ...
    async def old_value_update_pwd(cls, **kwargs):
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                user_admins = await session.execute(
                    select(SysUserAdminModel).where(
                        and_(SysUserAdminModel.password == old_password, SysUserAdminModel.emp_no == emp_no),
                    ),
                )
                user_admin = user_admins.scalars().first()
                if user_admin:
    ...
    async def add_security(**kwargs):
        ...
        await AsyncDbSession.begin_lock(
            pending_begin_number=len(pending_begin),
            min_begin_number=min_begin_number,
            max_begin_number=max_begin_number,
        )
        async with async_db_session_generator() as session:
            async with session.begin():
                sql = select(func.count(PikaSecurityRelIssues.id)).where(PikaSecurityRelIssues.emp_no == emp_no)
    ...
    async def empty_security(**kwargs):
        ...
        async with async_db_session_generator() as session:
            sel_res = await session.execute(
                select(PikaSecurityRelIssues.id).where(
                    and_(PikaSecurityRelIssues.id.in_(ids), PikaSecurityRelIssues.emp_no == emp_no),
                ),
            )
            sel_res_ids = sel_res.scalars().all()
            intersection = list(set(ids).difference({str(index) for index in sel_res_ids}))
            if not sel_res_ids:
                return PikaResponse.failed(detail="删除失败,id验签不通过")
            elif len(intersection) > 0:
                return PikaResponse.failed(detail="非管理员仅可删除自身的密保", data={"intersection": intersection})
        ...
        async with async_db_session_generator() as session:
            async with session.begin():
                sql = select(distinct(PikaSecurityRelIssues.id)).where(
                    and_(
                        PikaSecurityRelIssues.id.in_([index.id for index in pending_begin]),
                ...
                for pb in pending_begin:
                    if pb.id in exists_id:
                        sql = (
                            update(PikaSecurityRelIssues)
                            .where(PikaSecurityRelIssues.id == pb.id)
                            .values({"question": pb.question, "answers": pb.answers})
                        )
                        try:
                            await session.execute(sql)
                        except Exception as e:
            ...
            if len(failed) <= 0 and len(not_funded) <= 0:
                ...
                msg = "修改失败" if len(success) <= 0 and (len(failed) > 0 or len(not_funded) > 0) else "部分修改成功"
                return PikaResponse.success(
