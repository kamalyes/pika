...
class RoleDao:
    ...
    async def list(db, request: Any) -> Dict[Text, Any]:
        ...
        dim_do_sql = select(RoleModel).where(
            or_(
                ...
                and_(RoleModel.create_date >= request.create_date, RoleModel.update_date <= request.update_date),
            ),
        )
        ...
        try:
            ...
            async with async_db_session_generator() as session:
                async with session.begin():
                    ...
                    update_role_info_sql = (
                        update(RoleModel).where(RoleModel.id == id).values(**request.__dict__, operator=operator)
                    )
    ...
    async def delete(cls, id: str):
        try:
            async with async_db_session_generator() as session:
                async with session.begin():
                    query_us_role_sql = select(UserModel).where(UserModel.roles == id)
