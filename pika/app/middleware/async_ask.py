...
class AsyncRequest(PikaJsonEncoder):
    ...
    async def download(self):
        async with aiohttp.ClientSession(cookie_jar=aiohttp.CookieJar(unsafe=True)) as session:
            async with session.request(
                "GET", self.url, timeout=self.timeout, proxy=self.proxy, ssl=False, **self.kwargs,
            ) as resp:
                if resp.status != 200:
    ...
    async def collect(
        ...
        **kwargs,
        ...
        request_headers = cls.safe_json_dumps(
            dict(request_headers.items()) if request_headers is not None else {}, ensure_ascii=False,
        )
        response_headers = cls.safe_json_dumps(
            dict(response_headers.items()) if response_headers is not None else {}, ensure_ascii=False,
        )
