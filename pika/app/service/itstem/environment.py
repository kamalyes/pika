...
async def delete_environment(
    id: str, user_info=Depends(Permission(RoleEnum.ADMIN)), session=Depends(async_db_session_iterator),
):
...
async def update_environment(
    data: EnvironmentSchema, user_info=Depends(Permission(RoleEnum.ADMIN)), session=Depends(async_db_session_iterator),
):
...
async def list_environment(
    paging: BaseOnlyPagingSchema = Depends(), name: str = None, exactly=False, user_info=Depends(Permission()),
):
