...
async def insert_redis_config(form: RedisConfigSchema, user_info=Depends(Permission(RoleEnum.ADMIN))):
    try:
...
async def delete_redis_config(
    ...
    try:
        ans = await PikaRedisConfigDao.delete_record_by_id(
...
async def update_redis_config(
    form: RedisConfigSchema, background_tasks: BackgroundTasks, user_info=Depends(Permission(RoleEnum.ADMIN)),
):
    try:
        ...
        if result.cluster:
            ...
            background_tasks.add_task(
                PikaRedisManager.refresh_redis_client, *(result.id, result.addr, result.password, result.db),
            )
        return PikaResponse.success(data=result)
    except Exception as err:
        return PikaResponse.failed(detail=str(err))
...
async def list_redis_config(
    ...
    try:
        data = await PikaRedisConfigDao.select_list(
            name=PikaRedisConfigDao.like(name), addr=PikaRedisConfigDao.like(addr), env=env, cluster=cluster,
        )
