...
async def create_oss_file(filepath: str, file: UploadFile = File(...), user_info=Depends(Permission(RoleEnum.MANAGER))):
    try:
...
async def list_oss_file(filepath: str = None, user_info=Depends(Permission(RoleEnum.MANAGER))):
    try:
        records = await PikaOssDao.select_list(condition=[OssFileModel.file_path.like(f"%{filepath}%")])
...
async def delete_oss_file(
    filepath: str, user_info=Depends(Permission(RoleEnum.MANAGER)), session=Depends(async_db_session_iterator),
):
    try:
        # 先获取到本地的记录,拿到sha值
        record = await PikaOssDao.query_record(file_path=filepath, delete_flag=False)
