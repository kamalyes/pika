...
from app.schema.api_testcase import (
    TestCaseAssertsSchema,
    TestCaseGeneratorSchema,
...
from app.schema.constructor import ConstructorIndexSchema, ConstructorSchema
...
async def list_testcase(
    paging: BaseOnlyPagingSchema = Depends(), directory_id: str = None, name: str = None, operator: str = None,
):
...
async def create_testcase(
    data: TestCaseInfoSchema, user_info=Depends(Permission()), session=Depends(async_db_session_iterator),
):
...
async def update_testcase(form: TestCaseSchema, user_info=Depends(Permission())):
    try:
        ...
        return PikaResponse.success(data={"case_info": data, "out_parameters": result})
    except Exception as e:
        return PikaResponse.failed(detail=str(e))
...
async def delete_testcase(
    id_list: List[str], user_info=Depends(Permission()), session=Depends(async_db_session_iterator),
):
...
async def query_report(id: str, user_info=Depends(Permission())):
    ...
    return PikaResponse.success(data={"report": report, "plan_name": plan_name, "case_list": case_list})
...
async def list_report(
    ...
    report_list, total = await ApiTestReportDao.list_report(
        paging, request.start_date, request.finished_date, request.executor,
    )
...
async def get_directory_and_case(project_id: str, user_info=Depends(Permission())):
    ...
    return PikaResponse.success(data={"tree": tree_data, "case_map": cs_map})
...
async def delete_testcase_out_parameters(
    id: str, user_info=Depends(Permission()), session=Depends(async_db_session_iterator),
):
    await ApiTestCaseOutParametersDao.delete_record_by_id(
        session=session, operator=user_info["emp_no"], value=id, log=True,
    )
    return PikaResponse.success()
...
async def generate_case(
    form: TestCaseGeneratorSchema, user_info=Depends(Permission()), session=Depends(async_db_session_iterator),
):
