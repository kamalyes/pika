...
async def execute_sql(data: OnlineSqlSchema, escarole=Depends(Permission(escarole=True))):
    ...
    try:
        ...
        return PikaResponse.success(data={"result": result, "columns": columns, "elapsed": elapsed})
    except Exception as err:
        return PikaResponse.failed(detail=str(err))


@router.get("/sql/history/query", summary="获取sql执行历史记录")
async def query_sql_history(paging: BaseOnlyPagingSchema = Depends(), user_info=Depends(Permission())):
    data, total = await SQLHistoryDao.list_with_pagination(
        paging,
        _sort=[SQLHistoryModel.create_date.desc()],
        _select=[DatabaseModel, EnvironmentModel],
        _join=[
            (DatabaseModel, DatabaseModel.id == SQLHistoryModel.database_id),
            (EnvironmentModel, EnvironmentModel.id == DatabaseModel.env),
        ],
    )
    ans = []
    for history, database, env in data:
        ans.append({"history": history, "database": database, "env_info": env})
    return PikaResponse.success({"data": ans, "total": total})
...
async def list_tables(form: DatabaseSchema, user_info=Depends(Permission())):
    try:
        ...
        return PikaResponse.success({"children": children, "tables": tables})
    except Exception as err:
        return PikaResponse.failed(detail=str(err))
