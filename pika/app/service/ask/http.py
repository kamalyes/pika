...
async def http_request(data: HttpRequestSchema, user_info=Depends(Permission())):
    r = await AsyncRequest.client(
        data.url, data.request_body_type, headers=data.headers, request_body=data.request_body,
    )
...
async def http_request(cert: CertType):
    try:
        ...
        filename = f"{''.join((str(x) for x in shuffle))}mitmproxy.{suffix}"
...
async def re_run_case(
    env: str, case_id: str, data_id: str, retry_id: str, report_id: str, user_info=Depends(Permission()),
):
    try:
        executor = Executor()
        params = {}
        ...
        result = await executor.run_with_test_data(
            env=env, case_id=case_id, report_id=report_id, request_param=params, retry_id=retry_id,
        )
...
async def sync_execute_case(env: str, case_id: List[str], user_info=Depends(Permission())):
    data = {}
    uuid.uuid5(uuid.NAMESPACE_URL, "task")
    # s = time.perf_counter()
    for c in case_id:
        executor = Executor()
