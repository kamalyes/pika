...
from app.schema.kerberos import DelKerberosSchema, EditKerberosItemSchema, QueryKerberosInSchema, QueryKerberosOutSchema
...
async def add_encrypt_issue(security: EditKerberosItemSchema = Depends(), user_info=Depends(Permission())):
...
async def delete_encrypt_issue(request: DelKerberosSchema = Depends(), user_info=Depends(Permission())):
    return await KerberosDao.delete_encrypt_issue(request=request)


@router.post("/issue/update", summary="编辑推荐的密保问题")
async def update_encrypt_issue(security: EditKerberosItemSchema = Depends(), user_info=Depends(Permission())):
    return await KerberosDao.update_encrypt_issue(request=security, emp_no=user_info["emp_no"])


@router.get("/issue/list", summary="分页获取推荐的密保问题", response_model=LimitOffsetPage[QueryKerberosOutSchema])
async def query_encrypt_issue(
    request: QueryKerberosInSchema = Depends(),
