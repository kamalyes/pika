...
from app.schema.menu import EditMenuSchema, QueryMenuInSchema, QueryMenuOutSchema
from app.service import Permission

router = APIRouter()


@router.post("/menu/list", summary="获取所有菜单数据", response_model=LimitOffsetPage[QueryMenuOutSchema])
async def list_menu(
    request: QueryMenuInSchema = Depends(),
...
async def save_or_update_menus(
    request: EditMenuSchema,
    # user_info=Depends(Permission())
):
    try:
...
def delete_menu(id: BaseBatchDelIdsSchema, user_info=Depends(Permission())):
    pass
