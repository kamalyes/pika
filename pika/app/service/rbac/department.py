...
from app.schema.base import BaseBatchDelIdsSchema, BaseOnlyPagingSchema
...
async def delete_department(request: BaseBatchDelIdsSchema = Depends(), user_info=Depends(Permission(RoleEnum.ADMIN))):
...
async def update_department(
    ...
    await DepartmentDao.parity_field(
        session=session, name=form.name, organization_id=form.organization_id, dept_id=form.id, parent_id=form.parent_id,
    )
...
async def list_department(
    form: QueryDepartmentInSchema = Depends(),
