...
async def delete_project(
    project_id: str, escarole=Depends(Permission(RoleEnum.MANAGER, True)), session=Depends(async_db_session_iterator),
):
    operator, operator_identity = escarole
    try:
        async with session.begin():
            ...
            await ApiTestPlanDao.delete_record_by_id(
                session=session, operator=operator, project_id=project_id, key=project_id, session_begin=True,
            )
        return PikaResponse.success()
...
async def list_project(
    paging: BaseOnlyPagingSchema = Depends(), name: str = None, escarole=Depends(Permission(escarole=True)),
):
    operator, operator_identity = escarole
...
async def update_project_avatar(
    project_id: str, file: UploadFile = File(...), escarole=Depends(Permission(escarole=True)),
):
    try:
        operator, operator_identity = escarole
