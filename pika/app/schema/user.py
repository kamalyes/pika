...
class OAuth2TokenSchema:
    ...
    def __init__(
        self,
        emp_no: Optional[str] = Header(..., title="用户编码", max_length=ByteSizeEnum.LENGTH_20),
        token: Optional[str] = Header(..., title="token", max_length=ByteSizeEnum.LENGTH_600),
    ):
        self.emp_no = emp_no
        self.token = token
...
class RegisterUserSchema(BaseModel):
    ...
    el_code: Optional[str] = Body(None, title="验证码", max_length=ByteSizeEnum.LENGTH_11)
...
class OAuth2LoginSchema:
    ...
    def __init__(
        ...
        grant_type: Optional[str] = Form(
            ..., title="授权方式, account:用户名/员工编号、email:邮箱验证码", max_length=ByteSizeEnum.LENGTH_255,
        ),
