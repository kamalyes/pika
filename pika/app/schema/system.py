...
class RedisSchema(BaseModel):
    ...
    target_max_memory: Optional[str] = Body(
        PikaAppConfig.REDIS_TARGET_MAX_MEMORY, title="时区", max_length=ByteSizeEnum.LENGTH_36,
    )
    max_connections: Optional[str] = Body(
        PikaAppConfig.REDIS_MAX_CONNECTIONS, title="时区", max_length=ByteSizeEnum.LENGTH_36,
    )
