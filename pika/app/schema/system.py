...
class RedisSchema(BaseModel):
    ...
    encoding: Optional[str] = Body(
        PikaAppConfig.REDIS_ENCODING, title="链接的数据库名称", max_length=ByteSizeEnum.LENGTH_56
    )
    ...
    target_max_memory: Optional[str] = Body(
        PikaAppConfig.REDIS_TARGET_MAX_MEMORY,
        title="时区",
        max_length=ByteSizeEnum.LENGTH_36,
    )
    max_connections: Optional[str] = Body(
        PikaAppConfig.REDIS_MAX_CONNECTIONS,
        title="时区",
        max_length=ByteSizeEnum.LENGTH_36,
    )
