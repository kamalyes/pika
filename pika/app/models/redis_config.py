...
@Desc    :  redis配置
"""
from sqlalchemy import INT, Boolean, Column, ForeignKey, String, UniqueConstraint
...
class RedisModel(LargeBaseModel):
    ...
    __table_args__ = (UniqueConstraint("env", "name"), {"comment": "Redis配置"})
    env = Column(BinaryUUID, ForeignKey(EnvironmentModel.id, ondelete="cascade", onupdate="cascade"), comment="环境id")
    name = Column(String(ByteSizeEnum.LENGTH_50), nullable=False, comment="redis名称")
    addr = Column(String(ByteSizeEnum.LENGTH_128), nullable=False, comment="连接地址")
    username = Column(String(ByteSizeEnum.LENGTH_200), nullable=True, comment="用户名")
