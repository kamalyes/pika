...
from sqlalchemy import BOOLEAN, DATETIME, INT, Column, ForeignKey, String, UniqueConstraint
...
class ProjectModel(LargeBaseModel):
    ...
    def __init__(
        self, name, app, owner, operator, description="", private=False, avatar=None, dingtalk_url="", qy_wx_url="",
    ):
...
class ProjectRoleModel(LargeBaseModel):
    ...
    project_id = Column(
        BinaryUUID, ForeignKey(ProjectModel.id, ondelete="cascade", onupdate="cascade"), index=True, comment="项目id",
    )
...
class ProjectIterModel(LargeBaseModel):
    ...
    name = Column(String(ByteSizeEnum.LENGTH_16), unique=True, index=True, comment="迭代版本名称")
    project_id = Column(
        BinaryUUID, ForeignKey(ProjectModel.id, ondelete="cascade", onupdate="cascade"), index=True, comment="项目id",
    )
