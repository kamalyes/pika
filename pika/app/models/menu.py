...
class MenuModel(LargeBaseModel):
    ...
    is_link = Column(
        SMALLINT, nullable=True, comment="开启外链条件,`1、is_link: true 2、链接地址不为空(meta.is_link) 3、is_iframe: false`",
    )
    is_hide = Column(SMALLINT, nullable=True, default=False, comment="菜单是否隐藏(菜单不显示在界面,但可以进行跳转)")
