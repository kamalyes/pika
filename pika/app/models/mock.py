...
from sqlalchemy import INT, SMALLINT, Column, String, Text
...
class MockModel(LargeBaseModel):
    ...
    project_id = Column(BinaryUUID, default=uuid4, nullable=False, comment="项目id")
    url = Column(String(ByteSizeEnum.LENGTH_600), index=True, nullable=False, comment="url地址")
    method = Column(String(ByteSizeEnum.LENGTH_16), nullable=False, server_default="GET", comment="请求方式")
    headers = Column(Text, comment="headers")
    match_type = Column(SMALLINT, server_default="0", comment="类型:0:default,1:mockjs,2:faker")
    ...
    status_code = Column(INT, server_default="200", comment="http 响应状态码:200(默认)")
