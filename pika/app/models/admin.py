...
from sqlalchemy import SMALLINT, Column, DATETIME, String
...
class SysUserAdminModel(LargeBaseModel):
    ...
    is_activate = Column(SMALLINT, server_default="0", comment="激活状态,0:未激活、1:激活")
    ...
    err_pwd_count = Column(SMALLINT, server_default="0", comment="错误密码登录的次数")
    ...
    last_login_ip = Column(
        String(ByteSizeEnum.LENGTH_30), comment="最后一次登录所在IP")
    last_logout_ip = Column(
        String(ByteSizeEnum.LENGTH_30), comment="最后一次退出登录的所在IP")
    last_login_location = Column(
        String(ByteSizeEnum.LENGTH_30), comment="最后一次登录所在城市")
    last_login_date = Column(DATETIME, server_default=None, comment="最后一次登录时间")
    last_logout_date = Column(
        DATETIME, server_default=None, comment="最后一次退出登录时间")
